<x-guest-layout>
    <x-authentication-card>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <input type="checkbox" class="form-check-input @error('terms') is-invalid @enderror" value="true" name="terms" id="keep-signed" required {{ !old('terms') ?: 'checked' }}>
                <label class="form-check-label fs-sm">
                    {{ __('I agree to') }}
                <a class="nav-link-style fs-ms text-black-50 text-primary text-decoration-none" href="{{url('terms_and_conditions')}}">
                    {{ __('Terms & Conditions') }}
                </a>
            </label>
